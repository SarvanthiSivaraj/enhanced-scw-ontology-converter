...
class SCWProcessor:
    ...
    def _find_ontology_representative(self, partial_data: UnifiedConcept, missing_ontology_id: str) -> Optional[Dict]:
        ...
        # Initialize these for the scope of the loop iteration to prevent Pylance warnings
        pred_sim = 0.0
        prop_sim = 0.0
        lexical_sim = 0.0
        ...
        for concept_id, concept_data in self.unified_concepts.items():
            ...
            if candidate_entry_from_missing_ont:
                ...
                pred_sim = self._jaccard_similarity(sample_predicates, candidate_predicates)
                prop_sim = self._jaccard_similarity(sample_properties_fingerprint, candidate_properties_fingerprint)
                lexical_sim = self._jaccard_similarity(sample_terms, candidate_terms)
    ...
    def _phase8_save_and_validate(self, unified_graph: Graph, output_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save unified OWL
            unified_graph.serialize(destination=output_file, format='xml')
            logger.info(f"Unified ontology saved to: {output_file}")
    ...
    def _validate_unified_ontology(self, unified_graph: Graph, output_file_path: str) -> Dict[str, Any]:
        ...
        if self.config.get('enable_reasoning_validation', False):
            ...
            try:
                ...
                # Create a temporary output file for HermiT's results
                hermit_output_file = Path(output_file_path).parent / "hermit_reasoning_output.txt" # Fixed name for overwriting
        ...
        else: # Case where reasoning is disabled or required tools are missing/config does not enable subprocess
...
def create_processing_report(result: Dict[str, Any], output_path: str = "scw_report.html") -> str:
    """
    Create HTML processing report
    
    Args:
        result: SCW processing result
    ...
    <head>
        ...
        <title>SCW Processing Report</title>
        <style>
            /* Ensure valid CSS syntax - no leading/trailing spaces within quotes */
            ...
            .section { background-color: white; padding: 25px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); margin: 20px 0; }
            .section h2 { color: #007bff; margin-top: 0; border-bottom: 2px solid #eee; padding-bottom: 10px; margin-bottom: 20px; font-size: 1.8em;}
            .status-success { background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; padding: 10px; border-radius: 5px; text-align: center; font-weight: bold; }
            .status-error { background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; padding: 10px; border-radius: 5px; text-align: center; font-weight: bold; }
            .status-warning { background-color: #fff3cd; color: #856404; border: 1px solid #ffeeba; padding: 10px; border-radius: 5px; text-align: center; font-weight: bold; }

            .metric-container { display: flex; flex-wrap: wrap; justify-content: space-around; gap: 15px; margin-top: 20px; }
            .metric { flex: 1 1 calc(33% - 30px); min-width: 200px; background-color: #e9f0f8; padding: 15px; border-radius: 5px; text-align: center; box-shadow: inset 0 1px 3px rgba(0,0,0,0.05); }
            .metric-label { font-size: 0.9em; color: #555; }
            .metric-value { font-size: 1.8em; font-weight: bold; color: #007bff; margin-top: 5px; }
            table { width: 100%; border-collapse: collapse; margin: 20px 0; }
            th, td { padding: 12px 15px; text-align: left; border-bottom: 1px solid #ddd; }
            th { background-color: #f0f8ff; color: #333; font-weight: bold; }
            tr:nth-child(even) { background-color: #f9f9f9; }
            .code-block { background-color: #f4f4f4; border: 1px solid #ddd; border-left: 5px solid #007bff; padding: 15px; margin: 15px 0; overflow-x: auto; font-family: 'Courier New', monospace; font-size: 0.9em; }
            .error-list li { color: #dc3545; margin-bottom: 5px; }
            .warning-list li { color: #ffc107; margin-bottom: 5px; }
        </style>
    </head>
    <body>
        <div class="header">
            <h1>SCW Algorithm Processing Report</h1>
    ...
    # Check if processing was successful but there were unparsable files, which is a warning state
    elif result['status'] == 'success' and result.get('unparsable_files'): 
    ...
    if result['status'] == 'success':
        ...
        unparsable_files = result.get('unparsable_files', [])
        
        # Build quality metrics table rows
        quality_rows = ""
        quality_data = validation.get('quality_metrics', {})
        quality_rows += f"<tr><td>OWL Syntax Valid</td><td>{'✅ Yes' if quality_data.get('owl_syntax_valid', False) else '❌ No'}</td></tr>"
        
        # Adding preservation and completeness metrics
        for key, value in validation.get('preservation_metrics', {}).items():
            quality_rows += f"<tr><td>{key.replace('_', ' ').title()}</td><td>{value}</td></tr>"
        for key, value in validation.get('completeness_verification', {}).items():
            quality_rows += f"<tr><td>{key.replace('_', ' ').title()}</td><td>{value}</td></tr>"

        error_warnings_html = ""
        if quality_data.get('owl_errors_and_warnings'):
            error_warnings_html += "<h3>Validation Issues (Output Ontology):</h3><ul class='error-list'>"
            for err in quality_data['owl_errors_and_warnings']:
                error_warnings_html += f"<li>{err}</li>"
            error_warnings_html += "</ul>"
        
        unparsable_html = ""
        if unparsable_files:
            unparsable_html += "<h3>⚠️ Unparsable Input Files:</h3><ul class='warning-list'>"
            for uf in unparsable_files:
                unparsable_html += f"<li><strong>{uf['file_path']}</strong>: {uf['error']}</li>"
            unparsable_html += "</ul><p>These files were skipped and not included in the unified ontology.</p>"


        config_html = "<h3>Configuration Used:</h3><table><tr><th>Parameter</th><th>Value</th></tr>"
        for k, v in config_used.items():
            config_html += f"<tr><td>{k}</td><td>{v}</td></tr>"
        config_html += "</table>"

        concept_dist_html = "<h3>Concept Distribution:</h3><table><tr><th>Concept Type</th><th>Count</th></tr>"
        for k, v in quality_data.get('concept_distribution', {}).items():
            concept_dist_html += f"<tr><td>{k.replace('_', ' ').title()}</td><td>{v}</td></tr>"
        concept_dist_html += "</table>"
        
        rel_dist_html = "<h3>Relationship Distribution:</h3><table><tr><th>Relationship Type</th><th>Count</th></tr>"
        for k, v in quality_data.get('relationship_distribution', {}).items():
            rel_dist_html += f"<tr><td>{k.replace('_', ' ').title()}</td><td>{v}</td></tr>"
        rel_dist_html += "</table>"

        content = f"""
        <div class="section">
            <h2>Output File</h2>
            <p>Unified ontology created: <strong>{result['output_file']}</strong></p>
        </div>
        
        <div class="section">
            <h2>Input Processing Summary</h2>
            <div class="metric-container">
                <div class="metric">
                    <div class="metric-label">Input Files Attempted</div>
                    <div class="metric-value">{validation.get('input_statistics', {}).get('total_input_files_attempted', 'N/A')}</div>
                </div>
                <div class="metric">
                    <div class="metric-label">Successfully Loaded Ontologies</div>
                    <div class="metric-value">{validation.get('input_statistics', {}).get('successfully_loaded_ontologies', 'N/A')}</div>
                </div>
                <div class="metric">
                    <div class="metric-label">Unparsable Files Skipped</div>
                    <div class="metric-value">{len(unparsable_files)}</div>
                </div>
            </div>
            {unparsable_html}
        </div>
        ...
        <div class="section">
            ...
            <div class="metric-container">
                <div class="metric">
                    <div class="metric-label">Total Concepts (Loaded)</div>
                    <div class="metric-value">{stats.get('total_input_concepts', 'N/A')}</div>
                </div>
                <div class="metric">
                    <div class="metric-label">Unified Concepts (Output)</div>
                    <div class="metric-value">{stats.get('total_unified_concepts', 'N/A')}</div>
                </div>
                <div class="metric">
                    <div class="metric-label">Processing Time</div>
                    <div class="metric-value">{stats.get('processing_time_seconds', 'N/A')}s</div>
                </div>
                 <div class="metric">
                    <div class="metric-label">Concepts Per Second</div>
                    <div class="metric-value">{stats.get('concepts_per_second', 'N/A')}</div>
                </div>
            </div>
        </div>
        
        <div class="section">
            <h2>Quality & Completeness Metrics (Output Ontology)</h2>
            <table>
        ...
        <div class="section">
            <h2>Concept & Relationship Distributions</h2>
        ...
        """
    else: # If the algorithm failed completely (e.g., less than 2 files parsed)
        ...
        if unparsable_files:
            ...
            for uf in unparsable_files:
                unparsable_html += f"<li><strong>{uf['file_path']}</strong>: {uf['error']}</li>"
            unparsable_html += "</ul><p>Please fix these files or ensure at least 2 valid OWL files are provided.</p>"

        content = f"""
        <div class="section status-error">
            <h2>❌ Processing Failed</h2>
            <p>Error: <strong>{error_info}</strong></p>
            {unparsable_html}
        </div>
        """
...
def main():
    ...
    if result and result['status'] == 'success':
        ...
        if result.get('unparsable_files'):
            print(f"⚠️ Warning: {len(result['unparsable_files'])} input file(s) could not be parsed and were skipped.")
        ...
        if args.report:
            try:
                report_path = create_processing_report(result, args.report)
        ...
        if val_report.get('quality_metrics', {}).get('owl_errors_and_warnings'):
            print(f"  ‼️ Output OWL has validation issues/warnings. Check report for details.")
